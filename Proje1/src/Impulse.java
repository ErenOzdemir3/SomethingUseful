...
//	}
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (is1Turn) {
			shoot.interrupt();
		ball[0].applyForce(xM, yM, mForce);                   //mouseu gücünü ayarladıktan gitmesini sağlayan foksiyon bu olmadan mouse pressed de çalışmıyor
		mForce = 0;
			draw.repaint();
			}
		else 
	...
	public void mouseDragged(MouseEvent e) { //mouseın basılıyken olduğu noktaya göre ıstakanın dönmesi topun gitmesi ve çizginin gitmesini sağlıyor
		xM = e.getX();
		yM = e.getY();
		draw.repaint();
	}

	public void mouseMoved(MouseEvent e) { //mouseın basılı değilken  olduğu noktaya göre ıstakanın dönmesi topun gitmesi ve çizginin gitmesini sağlıyor
		xM = e.getX();
		yM = e.getY();
		draw.repaint();
	}
	...
	class Drawing extends JComponent {
		...
		public Drawing() {
			try {
				Table = ImageIO.read(new File("C:/Users/eren/eclipse-workspace/Proje1/img/pool5.PNG"));//masayı yazdırdığı yer
			} catch (IOException ex) {
				System.out.println("psdlf");
			}
			try {
				Cue = ImageIO.read(new File("C:/Users/eren/eclipse-workspace/Proje1/img/Cue.PNG")); //ıstakayı yazdırdığı yer
		...
		if(is1Turn) {
			...
			trans.translate(ball[0].x + 5, ball[0].y + 10);                                                   //rotate olmasını sağlayan kısım  CALIS ÖNEMLİ SORU GELİCEK
			trans.rotate(angle);
		...
		else
		{
				...
				trans1.translate(ball[1].x + 5, ball[1].y + 10);                                                  //rotate olmasını sağlayan kısım  CALIS ÖNEMLİ SORU GELİCEK
				trans1.rotate(angle);
